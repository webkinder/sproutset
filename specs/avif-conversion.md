...
**Variant generation (`AvifVariantGenerator`).** For an existing subsize file (`image-300x200.jpg`) the generator writes an AVIF sibling in the same directory via `wp_get_image_editor()->save($avifPath, 'image/avif')` with the configured quality. The sibling filename is scoped by attachment ID (`image-300x200.{attachmentId}.avif`, via `AvifSiblingPath`) rather than derived from the basename alone: WordPress permits two distinct attachments to share a basename across different extensions (e.g. `siegel.png` and `siegel.jpg`), and a basename-only sibling name would let both attachments resolve to, write, and delete the same AVIF file — corrupting or removing one attachment's AVIF as a side effect of generating or deleting the other's. It only runs when `enabled` **and** `AvifSupport::isSupported()`. Generation is **lazy and bounded**: it enforces its own per-request generation cap, mirroring `OnDemandSizeGenerator`, so a single page cannot trigger unbounded encoding; siblings not produced this request fill in on later requests, and the fallback `<img>` covers any gap so nothing ever looks broken. It refuses to encode in three cases: an **animated GIF** source (AVIF would drop the animation), a source whose editor cannot be created, and a result that is **not smaller** than the source (which is discarded, avoiding the pathology of an AVIF larger than the original). On any encode failure it trips a **per-attachment fuse** — a marker written to the attachment's metadata — and never retries that attachment; the component then silently serves the original. All generation runs inside a `try/catch(Throwable)` for boot safety.
...
**Lifecycle and cleanup (`AvifCleanup`).** AVIF siblings are sproutset-owned and are not listed in WordPress attachment metadata, so WordPress will not remove them. On `delete_attachment`, sproutset unlinks the attachment's `.avif` siblings, deriving them **deterministically** from the attachment's own metadata — the original file plus each subsize `file` in `wp_get_attachment_metadata()['sizes']`, run through the same attachment-ID-scoped `AvifSiblingPath` used by the generator. It deliberately does **not** wildcard-glob the base filename: WordPress dedups uploads (`image.jpg`, `image-1.jpg`), so a prefix glob would match a different live attachment's siblings and delete them. The attachment-ID scoping applies the same discipline to the deterministic path itself: without it, deleting one attachment (e.g. `siegel.jpg`) could unlink the AVIF sibling actually owned by a different attachment that happens to share a basename (e.g. `siegel.png`). The trade-off is that a rare orphan left by a prior image edit is not swept, but such orphans are unreferenced and harmless.
...
Scenario: Keeps AVIF siblings isolated between attachments that share a basename
  Given two attachments whose original files share a basename but differ in extension
  When AVIF siblings are generated for both, and then one attachment is deleted
  Then each attachment's AVIF sibling has a distinct path, and deleting one attachment leaves the other's sibling intact
...
| `Keeps AVIF siblings isolated between attachments that share a basename` | `tests/Unit/AvifSiblingPathTest.php`, `tests/Integration/AvifVariantGeneratorTest.php` → `test_generates_distinct_avif_siblings_for_attachments_sharing_a_basename`, `tests/Integration/AvifCleanupTest.php` → `test_does_not_remove_avif_siblings_of_another_attachment_sharing_a_basename` |
